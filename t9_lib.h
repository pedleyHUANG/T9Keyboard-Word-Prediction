...
// Edited Summer 2024 by Audrey Seo (just for clarity, no spec changes)
 ...
 * dictionary.
 * Note that this just declares the type exists -- you will need to flesh out 
 * the details in t9_priv.h.
 */
struct T9;
 ...
 * 
 * Post: If "word" was nonempty and contained only the lowercase letters a 
 * through z, and "word" was not already present in the dictionary, the 
 * specified dictionary "dict" has been updated to include this word. Otherwise,
 ...
 * 
 * The function identifies a list of words that satisfy the following qualities:
 *   - they are of the same length as the digits-only portion of the string, and
 *   - the are exactly represented on a phone keyboard by those digits.
 *
 * If this list is empty, NULL is returned. 
 * 
 * Otherwise, the Nth (zero-indexed, such that N=0 gives the first element) item
 * in the list, where "N" is the number of pound signs given in "nums", is 
 * returned. If there are fewer than N-1 items in the list, NULL is returned.
 */
